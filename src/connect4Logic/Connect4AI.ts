...
export default class Connect4AI {
  ...
  public getMove = (): number => {
    ...
    if (opponentWinMoves.length !== 0) {
      return opponentWinMoves[
        Math.floor(Math.random() * opponentWinMoves.length)
      ];
    }
  ...
  public alphaBeta = (
    ...
    if (
      depth === 0 ||
      currentBoard.isFull() ||
      currentBoard.hasWonOnSquare(
        currentOpponent,
        currentBoard.numChipsInCol(latestCol) - 1,
        latestCol
      )
    ) {
    ...
    let columns: number[];
    if (depth > 2) {
      // use nodesOrder so can cut off the subtree that are likely to be cut off earlier
      columns = this.nodesOrder(currentBoard, currentPlayer);
    } else {
      // dont use nodesOrder() nodes order for the deepest 2 level which ow slower
      columns = Array(currentBoard.width)
        .fill(0)
        .map((val, index) => index);
    }
  ...
  public nodesOrder = (board: Board, currentPlayer: DiscState): number[] => {
    ...
    while (arrayMax(scores) !== null) {
      // while scores has non null number
      const currentHighest = arrayMax(scores);
      const currentLowest = arrayMin(scores);
      let index: number;
      if (currentPlayer === this.player) {
        // maximaxing player get descending order
        index = scores.indexOf(currentHighest);
      } else {
        // opponent get accending order
  ...
  public evaluateBoard = (
    ...
    if (latestCol === null) {
      ...
      playerWin = board.hasWonOnSquare(
        this.player,
        board.numChipsInCol(latestCol) - 1,
        latestCol
      );
      opponentWin = board.hasWonOnSquare(
        this.opponent,
        board.numChipsInCol(latestCol) - 1,
        latestCol
      );
    ...
    if (playerWin) {
      ...
      const threatScore = new ThreatEvaluation(
        board,
        this.player
      ).getEvaluation();
      return (
        threatScore +
        (this.possibleConnectedness(board, this.player) -
          this.possibleConnectedness(board, this.opponent)) *
          POSSIBLE_CONNECTEDNESS_WEIGHT +
        (this.innerConnectedness(board, this.player) -
          this.innerConnectedness(board, this.opponent)) *
          INNER_CONNECTEDNESS_WEIGHT
      );
    }
  };
