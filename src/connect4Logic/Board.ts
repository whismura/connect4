...
export default class Board {
  public static getOpponent = (player: DiscState): DiscState => {
    return player === DiscState.PlayerTwo
      ? DiscState.PlayerOne
      : DiscState.PlayerTwo;
  };
  ...
  constructor(
    width: number,
    height: number,
    firstPlayer: DiscState,
    winNumber: number,
    moveHistoryInput?: number[]
  ) {
    this.width = width;
    this.height = height;
  ...
  public hasWon = (player: DiscState): boolean => {
    ...
    for (let x = -this.height + 1; x < this.width; x++) {
      for (
        let y = Math.max(-x, 0);
        y < Math.min(this.height, this.width - x);
        y++
      ) {
  ...
  public gameOver = (): boolean => {
    return (
      this.isFull() ||
      this.hasWon(DiscState.PlayerOne) ||
  ...
  public hasWonOnSquare = (
    ...
    let counter = 0;
    for (
      let j = Math.max(0, col - this.WIN_NUMBER + 1);
      j < Math.min(this.width, col + this.WIN_NUMBER);
      j++
    ) {
    ...
    counter = 0;
    for (
      let i = Math.max(0, row - this.WIN_NUMBER + 1);
      i < Math.min(this.height, row + this.WIN_NUMBER);
      i++
    ) {
  ...
  public undo = (): void => {
    ...
    this.boardArray[this.numChipsInCol(lastestCol) - 1][lastestCol] =
      DiscState.Empty;
  };
}
