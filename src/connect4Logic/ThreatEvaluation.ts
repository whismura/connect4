...
export default class ThreatEvaluation{
  ...
  // -2: letting the opponent win
  // null: col has no space
  public threatMoveScores = (currentBoard:Board, player:DiscState):Array<number|null>=>{
    const playerThreat = player===DiscState.PlayerOne? ThreatState.PlayerOneThreat: ThreatState.PlayerTwoThreat;
    const opponentThreat = player===DiscState.PlayerOne? ThreatState.PlayerTwoThreat: ThreatState.PlayerOneThreat;
    ...
    for(let i =0;i<this.width;i++){
      ...
      if(this.threatBoard.threatArray[chipsHeight][i]===playerThreat||
        this.threatBoard.threatArray[chipsHeight][i]===ThreatState.BothPlayersThreat){
        scores[i]= 2;
        continue;
      }
      // if the square above is opponent's threat, return -2
      if(chipsHeight!==this.height-1){
        if(this.threatBoard.threatArray[chipsHeight+1][i]===opponentThreat||
          this.threatBoard.threatArray[chipsHeight+1][i]===ThreatState.BothPlayersThreat){
          scores[i]= -2;
          continue;
        }
      }
  ...
  public threatMinimax = (currentBoard:Board,player:DiscState):number=>{
    ...
    for(let i=0;i<scores.length;i++){
      ...
      if(returnValue===null){
        ...
        if(currentPlayer===this.maximizingPlayer){
          returnValue = Math.max(returnValue,this.threatMinimax(copiedBoard,currentOpponent));
        } else if(currentPlayer===Board.getOpponent(this.maximizingPlayer)) {
          returnValue = Math.min(returnValue,this.threatMinimax(copiedBoard,currentOpponent));
        } else {
          // console.error("somethings wrong");
        }
      }
  ...
  public getEvaluation = ():number=>{    
    ...
    } else if(this.threatBoard.hasDoubleEffectiveThreat(Board.getOpponent(this.maximizingPlayer))){
      return -DOUBLE_THREAT;
    }
    ...
    } else if (this.currentMovePlayer===Board.getOpponent(this.maximizingPlayer)){
      const scores = this.threatMoveScores(this.board,Board.getOpponent(this.maximizingPlayer));
      if(arrayMax(scores)===2){
        return -INSTANT_WIN;
      }
      if(arrayMax(scores)===-2){
